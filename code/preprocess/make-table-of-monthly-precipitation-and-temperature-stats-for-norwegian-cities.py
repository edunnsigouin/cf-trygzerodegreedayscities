...
def calc_monthly_precipitation_climatology_per_city(clim_year, month, city, path_in, window=20):
    """
    Opens all 'rr' NetCDF files from (clim_year - window) up to clim_year, 
    uses 'preprocess' to subselect bounding box, average in space, and 
    keep only data from the requested month.
    Returns an xarray.Dataset (or DataArray) with the selected data.
    """
    ...
    def preprocess(ds):
        """
        1) Subselect bounding box and drop everything else
        2) Average in space
        3) Keep only data from requested month
        Everything is done at the single-file level, prior to combining
        in open_mfdataset.
        """
        # --- bounding box ---
        mask = (
            ...
            & (ds["lon"] >= bbox["lon_min"]) 
            & (ds["lon"] <= bbox["lon_max"])
        ).compute()  # compute so we don't index with unknown-size Dask arrays
        ...
        # --- time selection ---
        # Here, time.dt.month is still a Dask array. To avoid errors, compute the mask:
        month_mask = (ds.time.dt.month == month_num).compute()
        ds = ds.sel(time=month_mask)
        
        return ds
    
    # Now open all files with custom preprocess
    ds = xr.open_mfdataset(filenames, preprocess=preprocess)
...
if __name__ == "__main__":
    ...
    ds = calc_monthly_precipitation_climatology_per_city(2025,month,'Bergen',path_in,window=20)
