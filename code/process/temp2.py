...
def compute_zero_degree_crossing(ds, include_precipitation=True):
    """Compute daily event flags, validity, and mean temperature."""
    valid_temperature = np.isfinite(ds["tn"]) & np.isfinite(ds["tx"])
    valid = valid_temperature
    ...
    tmean = xr.where(valid_temperature, 0.5 * (ds["tn"] + ds["tx"]), np.nan)

    return xr.Dataset(
        {
            "crossing": crossing.astype(np.int16),
            "valid": valid.astype(np.int16),
            "tmean": tmean.astype(np.float32),
        }
    )


def aggregate_crossing_by_season(ds_cross, seasons):
    """Aggregate zero crossings and mean temperature by season."""
    seasonal_stats = []
    ...
    for season_name in seasons:
        ...
        tmean = ds_season["tmean"].mean("time", skipna=True).astype(np.float32)
...
def add_output_metadata(ds_out, dataset_name, season_name, include_precipitation=True):
    ...
    ds_out["tmean"].attrs = {
        ...
        "units": "degC",
        "description": (
            "Seasonal mean of daily mean temperature, where daily mean temperature "
            "is calculated as (tn + tx) / 2 using days with finite tn and tx."
        ),
    }
